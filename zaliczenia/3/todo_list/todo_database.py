...
from todo_io import (load_todos, save_todos_csv, load_todos_from_csv,
                     load_todos_from_json, save_todos_json)
...
class ToDoDatabase:
    ...
    def get_todo_by_user_id(self, user_id):
        return [
            todo for todo in self._todos if
            (todo.user_id == user_id or todo.user_id == int(user_id))
        ]

    def get_not_done_by_user_id(self, user_id):
        return [
            todo for todo in self._todos if
            (todo.user_id == user_id or todo.user_id ==
             int(user_id)) and
    ...
    def load_todos_from_file(self):
        try:
            with open(self._path, "r") as filehandle:
                ...
                match ext:
                    case "json":
                        self._todos = load_todos_from_json(
                            filehandle)
                    case "csv":
                        self._todos = load_todos_from_csv(
                            filehandle)
    ...
    def calculate_done(self):
        return len([
            todo for todo in self._todos if todo.completed is True
        ])

    def calculate_not_done(self):
        return len([
            todo for todo in self._todos if todo.completed is False
        ])
