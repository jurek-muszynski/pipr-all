...
class ComplexNumber():
    ...
    def __truediv__(self, num_to_divide_by):
        if (num_to_divide_by.im == 0):
            return ComplexNumber(self.__re/num_to_divide_by.re,
                                 self.__im / num_to_divide_by.re)
        denominator_conjugate = ComplexNumber(
            num_to_divide_by.re, num_to_divide_by.im).conjugate()
        numerator = self * denominator_conjugate
        denominator = num_to_divide_by * denominator_conjugate
        return ComplexNumber(numerator.re/denominator.re,
